...
def plot_boxplot_comparison0(
    ...
    fig.add_shape(
        type="line",
        x0=-0.4,
        y0=population_stats.get("mean", 0),
        x1=0.4,
        y1=population_stats.get("mean", 0),
        line=dict(
            color="green",
            width=2,
            dash="dash",
...
def plot_comparative_shap(df_global_shap, df_client_shap, top_n=10):
    ...
    top_features = (
        df_global_shap.groupby("feature")["shap_value"]
...
def plot_shap_by_decision(df_long, client_data, target_column="TARGET", top_n=10):
    ...
    df_decision_shap = (
        df_long.groupby(["feature", "decision"])["shap_value"]
        .mean()
        .unstack()
        .fillna(0)
    )
