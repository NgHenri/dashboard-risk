...
from utils.shap_utils import load_model_artifacts, fetch_client_data_for_shap, load_test_data_from_api, get_top_positive_negative_features
...
def display_client_info_grid(client: dict):
    ...
    with st.container():
        ...
        df_infos = pd.DataFrame([
            {"Libellé": "Âge", "Valeur": format_years(client['DAYS_BIRTH']), "Afficher": False},
            {"Libellé": "Genre", "Valeur": "Homme" if client["CODE_GENDER"] == 1 else "Femme", "Afficher": True},
            {"Libellé": "Type de logement", "Valeur": "Locataire" if client["NAME_HOUSING_TYPE_RENTED_APARTMENT"] == 1 else "Autre", "Afficher": False},
            {"Libellé": "Statut marital", "Valeur" : "Marié(e)" if client["NAME_FAMILY_STATUS_MARRIED"] == 1 else "Autre", "Afficher": False},
            {"Libellé": "Type d'emploi", "Valeur": get_employment_type(client), "Afficher": False},
            {"Libellé":  "Stabilité professionnelle", "Valeur":format_percentage(client["DAYS_EMPLOYED_PERC"]), "Afficher": False},
            {"Libellé": "Revenu par personne", "Valeur": format_currency(client['INCOME_PER_PERSON']), "Afficher": True},
            {"Libellé": "Montant crédit", "Valeur": format_currency(client['AMT_CREDIT']), "Afficher": False},
            {"Libellé": "Charge crédit", "Valeur": format_percentage(client['INCOME_CREDIT_PERC']), "Afficher": False},
    ...
    try:
        ...
        with col2:
            ...
            with st.spinner("Génération de l'analyse..."):
                ...
                def plot_feature_distribution(dataframe, client_row, feature_name, color, title):
                    fig = px.histogram(
                ...
                dist1 = plot_feature_distribution(df_test, client_data, selected_pos_feature, "crimson", f"Distribution de {selected_pos_feature}")
                dist2 = plot_feature_distribution(df_test, client_data, selected_neg_feature, "seagreen", f"Distribution de {selected_neg_feature}")
